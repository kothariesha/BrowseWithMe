...
class ColorExtractor(object):
    def get_color(self, pixel_list, quality=10):
        ...
        :return tuple: (r, g, b)
        """
        palette = self.get_palette(pixel_list, 5, quality)
        return palette[0]

    def get_palette(self, pixel_list, color_count=10, quality=10):
        ...
        cmap,cmap_count = MMCQ.quantize(pixel_list, color_count)
	cmap_count = np.array(cmap_count)
	palette = cmap.palette
	cmap_count = 100.0*cmap_count/np.sum(cmap_count)
	colors = []
	
	for count, color_tuple in enumerate(palette):
		color = {}
		color['r'] = color_tuple[0]
		color['g'] = color_tuple[1]
		color['b'] = color_tuple[2]
		color['percentage'] = round(cmap_count[count],2)
...
class MMCQ(object):
	...
	cmap_count = []
	    ...
	    pq2_color = pq2.pop()
	    cmap.push(pq2_color)
	    cmap_count.append(pq2_color.count)
	    #cmap.push(pq2.pop())
        return [cmap, cmap_count]
